...
bool isCorrect(Question& _question, Response choice)
{
...
void displayQuestion(Question& _question, Response _response[],int arrSize)
{   
    ...
    int choice;
    cin >> choice;
    cout << "\nYou have selected: " << choice-1 << endl;
    if (isValidChoice(choice -1, arrSize)!= false)
    {
        isCorrect(_question, _response[choice-1]);
    }
    else 
    {
...
int main() 
{
    ...
    displayQuestion(*qptr, responses, arrSize);
